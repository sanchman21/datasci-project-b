...
# Training loop
data_type = 'neutrophil_clinical' # neutrophil_clinical, monocyte_clinical
...
num_rows = train_dataset.frame.shape[0]
ones = train_dataset.frame["morphology"].sum()
zeroes = num_rows - ones
count = torch.tensor([zeroes, ones])
class_weights = num_rows / count

# weights = utils.make_weights_for_balanced_classes(train_dataset.frame['morphology'], device)
weighted_sampler = sampler.WeightedRandomSampler(class_weights, len(class_weights))
