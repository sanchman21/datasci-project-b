...
for set_id in range(5):
    ...
    output_dir = f'./experiments/{data_type}/{data_type}_fold_{set_id}_with_TTA'
    ...
    use_scheduler = True
    optimizer = optim.SGD(model.parameters(), lr=2e-5, weight_decay=1e-4, momentum=0.9)
...
use_scheduler = True
final_optimizer = optim.SGD(model.parameters(), lr=2e-5, weight_decay=1e-4, momentum=0.9)
end_factor = 1e-5/1e-3
final_scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1, end_factor=end_factor, total_iters=num_epochs)
...
torch.save(final_model.state_dict(), os.path.join(model_dir, 'final.pth'))
